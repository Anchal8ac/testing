...
export default function AddPost() {
  ...
  useEffect(() => {
    localStorage.setItem(dataKey, JSON.stringify(posts));
  }, [posts]);
  ...
  const handleAddOrUpdate = async () => {
    ...
    try {
      const res = await axios.post("http://localhost:3000/addnote/add", {
        title,
        content,
      });
      ...
      <div className="max-w-6xl mx-auto bg-white rounded-2xl shadow-lg p-6 md:p-10">
        ...
        {posts.length === 0 ? (
          <p className="text-center text-gray-500 mt-6">
            No posts yet. Click "Add Post" to get started. ✍️
          </p>
        ) : (
